                ...
                mRender: function (data, type, full) {
                    html = '                                                                \
                            <div id="delete{0}_icon" class="inner_margin2 icon_button">     \
                                <i class="fa {1} fa-lg" style="color: {2};"></i>            \
                            </div>                                                          \
...
Messages.OpenMessage = function(p_msgID)
{
    ...
    $.ajax({
        ...
        success: function(p_data) {         //cl(p_data);
            $("#m2_strong").text(p_data.subject);
            $("#msgCtrl_group").show();
            $("#messages_list").html(null);
            $("#messages_list").show();
            
            Messages.DisplayMessage(p_data, 0);
            Messages.PrepareReply(p_data);
            
            Messages.OPEN_MSGID = p_data.id;
            Messages.OPEN_THDID = -1;
        },
...
Messages.OpenConversation = function(p_thdID)
{
    ...
    $.ajax({
        ...
        success: function(p_data) {
            var subj = p_data[p_data.length-1].subject;
            $("#m2_strong").text(subj);
            $("#msgCtrl_group").show();
            $("#messages_list").html(null);
            $("#messages_list").show();
            ...
            $.each(p_data, function(index, value) {
                ...
                if (index == 0)
                    order = 1;
                else if (index == p_data.length -1)
                    order = 3;
                Messages.DisplayMessage(value, order);                
            });
            
            var lastMsg = p_data[p_data.length -1];
